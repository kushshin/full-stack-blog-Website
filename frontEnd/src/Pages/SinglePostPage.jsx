...
import { LikePost, DisLikePost, AddComments, DeleteComments, DeletePost } from '../API Services/PostAPI'
...
const disLikePost = async (postId) => {
  ...
  const Comments = async (id) => {
    try {
      ...
      setNewPost(res.data.comment) // backend should return updated post with comments
      setComment("");
    } catch (error) {
      console.log(error)
    }
  ...
  const deleteComment = async (postid, commentid) => {
    try {
      ...
      setNewPost(res.data.comment) // updated post
    } catch (error) {
      console.log(error)
    }
  }
            ...
            <div key={id} className='border-2 w-[90%] flex flex-col gap-4 rounded-lg p-2'>
              <div className='flex gap-4 items-center'>
                ...
                {username === comment.username &&
                  <button onClick={() => deleteComment(newPost._id, comment._id)} className='text-red-500'>
                    <FaRegTrashAlt />
                  </button>
                }
              </div>
